...
var ChainValidation = {
  ...
  required: function required(value) {
    ...
    if (ChainValidation.is_empty(value)) {
      throw new Error("value required for ".concat(field_name, ": ").concat(value));
    }
  ...
  is_valid_symbol_error: function is_valid_symbol_error(value) {
    ...
    if (value.split('.').length > 2) {
      return "".concat(suffix, "have only one dot.");
    }
    ...
    if (!/[A-Z]$/.test(value)) {
      return "".concat(suffix, "end with a letter");
    }

    if (/^[A-Z0-9\.]$/.test(value)) {
      return "".concat(suffix, "contain only letters numbers and perhaps a dot.");
    }
