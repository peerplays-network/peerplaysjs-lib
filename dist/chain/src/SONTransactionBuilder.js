...
var SONTransactionBuilder = function () {
  function SONTransactionBuilder() {
    var privKey = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;

    _classCallCheck(this, SONTransactionBuilder);

    console.log('SONTransactionBuilder init..');
    ...
    console.log('privKey passed: ' + this.objToString(this.privKey));
  ...
  SONTransactionBuilder.prototype.add_type_operation = function add_type_operation(name, operation) {

    // Checking if SON ( HIVE ) 
    console.log('????? add_type_operation ?????: ' + JSON.stringify(operation));
    // if from or to has 'son-account' then we are on son operation
    //console.log('***** add_type_operation *****: ' + JSON.stringify(operation.amount.asset_id));

    // For now HBD asset_id = '1.3.2'
    // Later add SON GROUP ( we should group name getting such as get_asset_group(id) --> SON_HIVE

    // I used local compare for later to check if SON* with string locale utf8/16.
    //if(JSON.stringify(operation.amount.asset_id) === '1.3.2') {
    // For now son-account is '1.2.19' on testnet
    console.log('...CHANGING FOR SON...!!!!!!!!!!! 1234');

    //var ourmemo = JSON.stringify(operation.to);
    // eslint-disable-next-line max-len
    //operation.memo.message = Buffer.isBuffer(ourmemo) ? ourmemo : Buffer.concat([Buffer.alloc(4), Buffer.from(ourmemo.toString('utf-8'), 'utf-8')])
    //operation.memo.to = 'TEST1111111111111111111111111111111114T1Anm';
    //operation.to = '1.2.19';

    //let plaintext = Aes.decrypt_with_checksum(receiver, 
    //  sender.toPublicKey(), nonce, operation.message);
    ...
    */
    //console.log('IMPORTANT!!!!!! ' + operation.memo.nonce);
    //console.log('IMPORTANT!!!!!!222 ' + this.objToString(this.privKey));
    //console.log('this.privKey: ' + Object.prototype.toString.call(this.privKey));


    //let new_message = Aes.encrypt_with_checksum(this.privKey,
    //operation.memo.message_memopubkey, 
    //operation.memo.nonce, operation.memo.message_memo);

    //console.log('IMPORTANT 222222222  new_message: ' + new_message);

    console.log('===== OPERATION CHANGED: ===== ' + JSON.stringify(operation));
  ...
  SONTransactionBuilder.prototype.process_transaction = function process_transaction(cwallet) {
    var _this = this;
  ...
  SONTransactionBuilder.prototype.finalize = function finalize() {
    var _this2 = this;
  ...
  SONTransactionBuilder.prototype.id = function id() {
    if (!this.tr_buffer) {
  ...
  SONTransactionBuilder.prototype.add_operation = function add_operation(operation) {
    if (this.tr_buffer) {
      throw new Error('already finalized');
    }
  ...
  SONTransactionBuilder.prototype.get_type_operation = function get_type_operation(name, operation) {
    if (this.tr_buffer) {
      throw new Error('already finalized');
    }
  ...
  SONTransactionBuilder.prototype.update_head_block = function update_head_block() {
    var _this3 = this;
  ...
  SONTransactionBuilder.prototype.set_expire_seconds = function set_expire_seconds(sec) {
    if (this.tr_buffer) {
      throw new Error('already finalized');
    }
  ...
  SONTransactionBuilder.prototype.propose = function propose(proposal_create_options) {
    if (this.tr_buffer) {
      throw new Error('already finalized');
    }
  ...
  SONTransactionBuilder.prototype.has_proposed_operation = function has_proposed_operation() {
  ...
  SONTransactionBuilder.prototype.set_required_fees = function set_required_fees(asset_id) {
    var _this4 = this;
  ...
  SONTransactionBuilder.prototype.get_potential_signatures = function get_potential_signatures() {
  ...
  SONTransactionBuilder.prototype.get_required_signatures = function get_required_signatures(available_keys) {
  ...
  SONTransactionBuilder.prototype.add_signer = function add_signer(private_key) {
  ...
  SONTransactionBuilder.prototype.sign = function sign() {
  ...
  SONTransactionBuilder.prototype.broadcast = function broadcast(was_broadcast_callback) {
    var _this5 = this;
  ...
  SONTransactionBuilder.prototype._broadcast = function _broadcast(was_broadcast_callback) {
    var _this6 = this;
  ...
  SONTransactionBuilder.timeStringToDate = function timeStringToDate(time_string) {
    if (!time_string) {
  ...
  SONTransactionBuilder.prototype.base_expiration_sec = function base_expiration_sec() {
  ...
  SONTransactionBuilder.prototype.getHeadBlockDate = function getHeadBlockDate() {
    return SONTransactionBuilder.timeStringToDate(this.head_block_time_string);
  };

  SONTransactionBuilder.prototype.serialize = function serialize() {
    return _serializer.ops.signed_transaction.toObject(this);
  };

  SONTransactionBuilder.prototype.toObject = function toObject() {
    return _serializer.ops.signed_transaction.toObject(this);
  };

  SONTransactionBuilder.prototype.objToString = function objToString(obj) {
    var str = '';
  ...
  return SONTransactionBuilder;
}();

exports.default = SONTransactionBuilder;
