  ...
  _createClass(TransactionBuilder, [{
    ...
    value: function process_transaction(cwallet) {
      ...
      if (_ws.Apis.instance().chain_id !== wallet_object.get('chain_id')) {
        ...
        var error = new Error("Mismatched chain_id; expecting ".concat(wallet_chain_id, ", but got ").concat(api_chain_id));
          ...
          return _this.get_required_signatures(my_pubkeys).then(function (required_pubkeys) {
            ...
            for (var _i = 0, _len = keys.length; _i < _len; _i++) {
              ...
              if (!signer_pubkeys_added[_pubkey_string]) {
                ...
                if (!_private_key) {
                  throw new Error("Missing signing key for ".concat(_pubkey_string));
                }
    ...
    value: function sign() {
      ...
      for (var i = 0; end > 0 ? i < end : i > end; end > 0 ? i++ : i++) {
        var _this$signer_private_ = _slicedToArray(this.signer_private_keys[i], 2),
    ...
    key: "_broadcast",
    value: function _broadcast(was_broadcast_callback) {
      var _this6 = this;
      ...
      return new Promise(function (resolve, reject) {
        ...
        _ws.Apis.instance().network_api().exec('broadcast_transaction_with_callback', [function (res) {
          ...
          reject(new Error("".concat(message, "\n") + 'peerplays-crypto ' + " digest ".concat(_ecc.hash.sha256(_this6.tr_buffer).toString('hex'), " transaction ").concat(_this6.tr_buffer.toString('hex'), " ").concat(JSON.stringify(tr_object))));
        });
      });
    }
  }, {
    ...
    key: "base_expiration_sec",
    value: function base_expiration_sec() {
