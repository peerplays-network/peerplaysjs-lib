...
var _internal = {
  ...
  to_long64: function to_long64(number_or_string, precision) {
    ...
    _SerializerValidation["default"].required(number_or_string, "number_or_string ".concat(error_info));

    _SerializerValidation["default"].required(precision, "precision ".concat(error_info));
  ...
  decimal_precision_string: function decimal_precision_string(number, precision) {
    ...
    _SerializerValidation["default"].required(number, "number ".concat(error_info));
    ...
    if (!number_parts) {
      throw new Error("Invalid number: ".concat(number_string, " ").concat(error_info));
    }
    ...
    if (zero_pad_count < 0) {
      throw new Error("overflow, up to ".concat(precision, " decimals may be used ").concat(error_info));
    }
