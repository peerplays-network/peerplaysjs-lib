...
var Aes = /*#__PURE__*/function () {
  ...
  _createClass(Aes, [{
    ...
    value:
    /** @private */
    function _decrypt_word_array(cipher) {
    ...
    value: function fromSha512(hash) {
      _assert["default"].equal(hash.length, 128, "A Sha512 in HEX should be 128 characters long, instead got ".concat(hash.length));
    ...
    value: function fromBuffer(buf) {
      ...
      _assert["default"].equal(buf.length, 64, "A Sha512 Buffer should be 64 characters long, instead got ".concat(buf.length));
    ...
    value: function decrypt_with_checksum(private_key, public_key, nonce, message) {
      ...
      Buffer.from("".concat(nonce)), Buffer.from(S.toString('hex'))]));
    ...
    value: function encrypt_with_checksum(private_key, public_key, nonce, message) {
      ...
      var aes = Aes.fromSeed(Buffer.concat([// A null or empty string nonce will not effect the hash
      Buffer.from("".concat(nonce)), Buffer.from(S.toString('hex'))])); // DEBUG console.log('... S',S.toString('hex'))
