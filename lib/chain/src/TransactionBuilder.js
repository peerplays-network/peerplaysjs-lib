...
class TransactionBuilder {
  ...
  add_signer(private_key, public_key = null) {
    ...
    if(Object.keys((private_key)[0] === "active" || Object.keys(private_key)[0] === "memo") && WhaleVaultConfig.getWhaleVault() !== null) { 
  ...
  sign(chain_id = Apis.instance().chain_id) {
    ...
    for (let i = 0; end > 0 ? i < end : i > end; end > 0 ? i++ : i++) {
      ...
      if(Object.keys((private_key)[0] === "active" || Object.keys(private_key)[0] === "posting") && WhaleVaultConfig.getWhaleVault() !== null) {

        const keyType = Object.keys(private_key)[0];
        const messageBuffer = Buffer.concat([Buffer.from(chain_id, 'hex'), this.tr_buffer]);

        WhaleVaultConfig.getWhaleVault()
          .promiseRequestSignBuffer('peerplaysjs', `ppy:${private_key[keyType]}`, JSON.stringify(messageBuffer), keyType, this.operations[0][0], 'raw')
          .then(response => {
            if (response.success) {
