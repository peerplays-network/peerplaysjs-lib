...
import {WhaleVaultConfig} from '../../whaleVault';
...
class TransactionBuilder {
        ...
        Apis.instance()
          ...
          .then((r) => {
            ...
            if (this.expiration === 0) {
              this.expiration =
                this.base_expiration_sec() + ChainConfig.expire_in_secs;
            }
    ...
    if (name === 'proposal_create') {
      ...
      if (!operation.expiration_time) {
        ...
        operation.expiration_time =
          experationTime + ChainConfig.expire_in_secs_proposal;
      }
  ...
  add_signer(private_key, public_key = null) {
    if (
      Object.keys(
        private_key[0] === 'active' || Object.keys(private_key)[0] === 'posting'
      ) &&
      WhaleVaultConfig.getWhaleVault() !== null
    ) {
  ...
  signWithWhaleVault(chain_id = Apis.instance().chain_id) {
    if (!this.tr_buffer) {
      throw new Error('not finalized');
    }
    ...
    for (let i = 0; end > 0 ? i < end : i > end; end > 0 ? i++ : i++) {
      let private_key = this.signer_private_keys[i];
      const keyType = Object.keys(private_key)[0];

      let promise = WhaleVaultConfig.getWhaleVault().promiseRequestSignBuffer(
        'peerplaysjs',
        `ppy:${private_key[keyType]}`,
  ...
  _broadcast(was_broadcast_callback) {
    if (
      (Object.keys(this.signer_private_keys[0])[0] === 'active' ||
        Object.keys(this.signer_private_keys[0])[0] === 'posting') &&
      WhaleVaultConfig.getWhaleVault() !== null
    ) {
      return new Promise((resolve, reject) => {
        if (!this.signed) {
          this.signWithWhaleVault().then((responses) => {
            responses.forEach((response) => {
              if (response.success) {
                this.signatures.push(Buffer.from(response.result, 'hex'));
              } else {
                console.error(response);
                throw new Error(
                  `whalevault: ${response.error} [${response.message}]`
                );
              }
            });
                  ...
                  new Error(
                    `${message}\n` +
                      ...
                      ` digest ${hash
                        ...
                        .toString('hex')} transaction ${this.tr_buffer.toString(
                        'hex'
