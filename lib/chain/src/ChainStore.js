...
class ChainStore {
  ...
  clearCache() {
    ...
    this.lotteries_ids = Immutable.List();
        ...
        .then(
          (results) => {
            ...
            let {
              ...
              votes,
              proposals
            // file deepcode ignore OverwriteAssignment: <please specify a reason of ignoring this>
            } = full_account;
  ...
  _updateObject(object, notify_subscribers = false, emit = true) {
    ...
    */
    ...
    if (objectSpace === balance_prefix) {
      ...
      //lottery asset
      if (object.lottery_options && this.lotteries_ids.findIndex((item) => {
        return item === object.id;
      }) === -1) {
        this.lotteries_ids = this.lotteries_ids.push(object.id)
          .sortBy((item) => -parseInt(item.split('.')[2]));
      }
  ...
  static timeStringToDate(time_string) {
    if (!time_string) {
      const newLocal = new Date('1970-01-01T00:00:00.000Z');
      return newLocal;
    }
  ...
  getLastIdLottery() {
    ...
    return Apis.instance().db_api().exec('get_lotteries', ['1.3.0', 1, '1.3.0']).then((lotteries) => {
  ...
  getAllLotteriesIds() {
    ...
    return new Promise((resolve) => {
      ...
      if (this.lotteries_ids_initialized) {
        return resolve(this.lotteries_ids);
      }

      this.getLastIdLottery().then((lastId) => {

        let self = this;

        let fetchLotteries = function (prevId, limit, nextId, isLastRequest) {
          ...
          // eslint-disable-next-line max-len
          return Apis.instance().db_api().exec('get_lotteries', [prevId, limit, nextId]).then((lotteries) => {
            ...
            lotteries.forEach((lotteryAsset) => {
