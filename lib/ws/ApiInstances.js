...
class ApisInstance {
  ...
  connect(cs, connectTimeout) {
    ...
    this.init_promise = this.ws_rpc.login(rpc_user, rpc_password).then(() => {
      ...
      this._sidechain = new GrapheneApi(this.ws_rpc, 'sidechain');
      let db_promise = this._db.init()
        .then(() => this._db.exec('get_chain_id', [])
          .then((_chain_id) => {
      ...
      return Promise.all([
        ...
        this._bookie.init(),
        this._sidechain.init().catch((e) => console.error('ApiInstance\tSidechain API Error', e))
      ]);
    });
  }
