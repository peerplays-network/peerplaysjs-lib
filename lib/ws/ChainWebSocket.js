...
class ChainWebSocket {
  ...
  resetConnection() {
    ...
    if (this.currentReject) {
      this.currentReject(
        new Error(`Connection attempt failed: ${this.serverAddress}`)
      );
    }
  }
  ...
  call(params) {
    ...
    if (!this.healthCheck) {
      this.healthCheck = setTimeout(
        this.onConnectionTerminate.bind(this),
  ...
  close() {
    if (this.ws) {
      ...
      // Try and fire close on the connection.
        ...
        this.debug('Caught error when trying to close websocket: ' + ex);
      }
